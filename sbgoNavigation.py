...
def holeSubroutine()
    ...
    while(bot.read_single_distance() >= 10)
        bot.move_distance (1)
        holeLength += 1
    bot.move_distance(-(holeLength/2))
def movementLoop():
    bot.move_distance(16)#16cm is the width of the robot, so it's the minimum distance
    bot.turn_distance_sensor(0)#is this left?
    if(bot.read_single_distance() >= 10)
        holeSubroutine()
        bot.turn_wheels(1)
        movementLoop()#recursion!
    bot.turn_distance_sensor(180)#is this Right?
    if(bot.read_single_distance() >= 10)
        holeSubroutine()
        bot.turn_wheels(2)
        holeSubroutine()
    bot.turn_distance_sensor(90)#is this Right?
    if(bot.read_single_distance() >= 10)
        holeSubroutine()
    #dead end
    bot.turn_wheels(1)
