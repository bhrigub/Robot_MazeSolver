...
class MazeGame(object):
    ...
    def play(self):
        ...
        while player != target:
            ...
            move = {N: (0, -1), E: (1, 0), W: (-1, 0), S: (0, 1)}
            current_cell = self.maze[player]

            # make a decision point if less than 3 walls (ie, more than 1 way to
            # go) MAKE FANCY LOGIC HERE
            directions = self.get_directions(current_cell)
            direction = self.pick_direction(directions)
            #if len(current_cell.walls) < 3:
                # make decision point, at position x,y.  Choose direction to move
            #else:
                # only 1 way to go, so go that way 
            #if direction not in current_cell:
            #    player = (player[0] + difx, player[1] + dify)
            player = (player[0] + move[direction][0], player[1] + move[direction][1])
